...
public class SearchServiceImpl implements SearchService {
    ...
    public List<SearchResultDto> searchProjects(String keyword, Long userId) {
        ...
        return projects.stream().map(project -> {
            ...
            result.setMatchField(project.getName().toLowerCase().contains(keyword.toLowerCase()) ? "name" : "description");
            result.setHighlight(highlightKeyword(
                    project.getName().toLowerCase().contains(keyword.toLowerCase()) ? project.getName() : project.getDescription(),
                    keyword
            ));
            result.setRelevanceScore(calculateRelevanceScore(keyword, project.getName(), project.getDescription()));
            return result;
    ...
    public List<SearchResultDto> searchTasks(String keyword, Long userId) {
        ...
        return tasks.stream().map(task -> {
            ...
            result.setMatchField(task.getName().toLowerCase().contains(keyword.toLowerCase()) ? "name" : "description");
            result.setHighlight(highlightKeyword(
                    task.getName().toLowerCase().contains(keyword.toLowerCase()) ? task.getName() : task.getDescription(),
                    keyword
            ));
            result.setRelevanceScore(calculateRelevanceScore(keyword, task.getName(), task.getDescription()));
            return result;
        }).collect(Collectors.toList());
    }
    ...
    public List<SearchResultDto> searchFiles(String keyword, Long userId) {
        List<FileEntity> files = fileEntityMapper.selectList(new QueryWrapper<FileEntity>()
                .like("name", keyword)
                .orderByDesc("updated_at"));
        ...
        return files.stream().map(file -> {
            ...
            result.setDescription("文件大小: " + formatFileSize(file.getSize()));
            result.setUrl("/files/" + file.getId());
            result.setIcon("document");
            result.setUpdatedAt(file.getUpdatedAt());
            result.setMatchField("name");
            result.setHighlight(highlightKeyword(file.getName(), keyword));
            result.setRelevanceScore(calculateRelevanceScore(keyword, file.getName(), ""));
            return result;
        }).collect(Collectors.toList());
    }
    ...
    public List<SearchResultDto> searchUsers(String keyword, Long userId) {
        ...
        return users.stream().map(user -> {
            ...
            result.setMatchField(user.getName().toLowerCase().contains(keyword.toLowerCase()) ? "name" : "email");
            result.setHighlight(highlightKeyword(
                    user.getName().toLowerCase().contains(keyword.toLowerCase()) ? user.getName() : user.getEmail(),
                    keyword
            ));
            result.setRelevanceScore(calculateRelevanceScore(keyword, user.getName(), user.getEmail()));
            return result;
        }).collect(Collectors.toList());
    }
